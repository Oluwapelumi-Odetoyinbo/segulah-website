...
export const Card = forwardRef(({ customClass, className, children, ...rest }, ref) => (
  <div
    ref={ref}
    {...rest}
    className={`absolute top-1/2 left-1/2 rounded-xl border border-slate-100 bg-white transform-3d will-change-transform backface-hidden overflow-hidden ${customClass ?? ''} ${className ?? ''}`.trim()}
  >
    {children}
  </div>
));
